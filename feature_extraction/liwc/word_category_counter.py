...
def default_dictionary_filename():
    return os.path.abspath(os.path.join(os.path.dirname(__file__), 'LIWC2015_English_Flat.dic'))
...
def score_text(text, raw_counts=True, scores=None, unique_words=None):
    ...
    for line in text.strip().split('\n'):
        ...
        for i in range(len(all_bigrams)):
            pair = all_bigrams[i]
            if pair[0] in Dictionary.sentence_punctuation: continue
            if pair[1] in Dictionary.sentence_punctuation: continue
            bigram = pair[0] + " " + pair[1]
            if len(bigram) ==0: continue
            scores.update(_dictionary.score_word(bigram))

        all_trigrams = list(nltk.trigrams(all_tokens))
        for i in range(len(all_trigrams)):
            tri = all_trigrams[i]
            if tri[0] in Dictionary.sentence_punctuation: continue
            if tri[1] in Dictionary.sentence_punctuation: continue
            if tri[2] in Dictionary.sentence_punctuation: continue
            trigram = tri[0] + " " + tri[1] + " " + tri[2]
            if len(trigram) ==0: continue
            scores.update(_dictionary.score_word(trigram))
...
class Dictionary():
    ...
    def load_dictionary_file(self, filename, internal_category_list=None):
        ...
        for line in open(filename):
            ...
            cats = match.group('categories').split()
    ...
    def score_word(self, word, previous_word=None, next_word=None):
        ...
        if len(word) == 0:
            ...
            if not " " in word: scores['Word Count'] += 1
                    ...
                    for char in word:
                        if char in current_node:
                            ...
                            if isinstance(current_node, set):
                                if Dictionary._TYPE_BASIC not in self._lookup[word]:
                                    self._lookup[word][Dictionary._TYPE_BASIC]=set()
                                self._lookup[word][Dictionary._TYPE_BASIC].update(current_node) #add to doc2vec lookup for time efficiency
