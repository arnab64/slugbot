...
class discriminative:
	...
	def extractbigrams(self,name):
		fname='intermediate/pos_bigrams_'+name.lower()+'.txt'
		inf=open(fname,'r')
	...
	def extractunigrams(self,name):
		fname='intermediate/pos_unigrams_'+name.lower()+'.txt'
		inf=open(fname,'r')
		inl=inf.readlines()
		for line in inl:
			elx=line.split()
	...
	def print_bigrams(self,top):
		...
		ofile2=open('intermediate/friends_pos_discriminative_bigrams.txt','w')
		for key in self.pool.keys():
			elllx=self.pool[key]
			ofile1.write(str(key)+"	"+str(np.std(elllx))+"	"+str(elllx)+"\n")
			bigs.append((key,np.std(elllx)))
		...
		print("numbers are:",len(bigs))
		count=0
		print("selecting the top 20 discriminative!")
		for j in range(top):
			holx=bigs[j]
			bigr=holx[0]
			bigt=holx[1]
	...
	def print_unigrams(self,top):
		bigs=[]
		ofile1=open('intermediate/friends_unigram_relative_frequencies.txt','w')
		ofile2=open('intermediate/friends_pos_discriminative_unigrams.txt','w')
		for key in self.pool.keys():
			elllx=self.pool[key]
			ofile1.write(str(key)+"	"+str(np.std(elllx))+"	"+str(elllx)+"\n")
			bigs.append((key,np.std(elllx)))
